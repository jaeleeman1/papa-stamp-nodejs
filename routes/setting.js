...
router.get('/main', function(req, res, next) {
    ...
    if(userId.length > 0) {
        getConnection(function (err, connection){
            ...
            connection.query(selectUserInfo, function (err, userInfoData) {
                if (err) {
                    ...
                    res.status(200);
                    res.render('common/papa-stamp', {view:'setting', url:config.url, userId:userId, shopId:'', popupCheck:false, userInfoData:userInfoData[0]});
                }
                connection.release();
            });
        });
        ...
        res.status(200);
        res.render('common/papa-stamp', {view:'setting', url:config.url, userId:'', shopId:'', popupCheck:false, userInfoData:{USER_EMAIL:"papastamp@naver.com"}});
    }
